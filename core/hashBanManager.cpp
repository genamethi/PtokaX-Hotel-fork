        ...
        if(banlist != NULL) {
            ...
            if(bans != NULL) {
                ...
                while((child = bans->IterateChildren(child)) != NULL) {
                    ...
                    // PPK ... ipban
                    if(ipban == true) {
                        if(ip != NULL && HashIP(ip, pBan->m_ui128IpHash) == true) {
                            strcpy(pBan->m_sIp, ip);
                            pBan->m_ui8Bits |= IP;
                    ...
                    if(nickban == true) {
                        if(nick != NULL) {
                            ...
                            memcpy(pBan->m_sNick, nick, szNickLen);
                            pBan->m_sNick[szNickLen] = '\0';
                            pBan->m_ui32NickHash = HashNick(pBan->m_sNick, strlen(pBan->m_sNick));
                            pBan->m_ui8Bits |= NICK;
                        } else {
                            delete pBan;
                    ...
                    if(reason != NULL) {
...
#ifdef _WIN32
                        pBan->m_sReason = (char *)HeapAlloc(ServerManager::m_hPtokaXHeap, HEAP_NO_SERIALIZE, szReasonLen+1);
#else
						pBan->m_sReason = (char *)malloc(szReasonLen+1);
#endif
                        if(pBan->m_sReason == NULL) {
                        ...
                        memcpy(pBan->m_sReason, reason, szReasonLen);
            ...
            if(rangebans != NULL) {
                ...
                while((child = rangebans->IterateChildren(child)) != NULL) {
                    ...
                    // PPK ... fromip
                    if(HashIP(ipfrom, pRangeBan->m_ui128FromIpHash) == true) {
                        strcpy(pRangeBan->m_sIpFrom, ipfrom);
                    } else {
                        delete pRangeBan;
                    ...
                    // PPK ... toip
                    if(HashIP(ipto, pRangeBan->m_ui128ToIpHash) == true && memcmp(pRangeBan->m_ui128ToIpHash, pRangeBan->m_ui128FromIpHash, 16) > 0) {
                        strcpy(pRangeBan->m_sIpTo, ipto);
                    } else {
                        delete pRangeBan;
                    ...
                    if(reason != NULL) {
...
#ifdef _WIN32
                        pRangeBan->m_sReason = (char *)HeapAlloc(ServerManager::m_hPtokaXHeap, HEAP_NO_SERIALIZE, szReasonLen+1);
#else
						pRangeBan->m_sReason = (char *)malloc(szReasonLen+1);
#endif
                        if(pRangeBan->m_sReason == NULL) {
                        ...
                        memcpy(pRangeBan->m_sReason, reason, szReasonLen);
                    ...
                    if(by != NULL) {
                        ...
                        memcpy(pRangeBan->m_sBy, by, szByLen);
                    ...
                    if(((pRangeBan->m_ui8Bits & TEMP) == TEMP) == true) {
                        ...
                        // PPK ... temp ban expiration
                        pRangeBan->m_tTempBanExpire = expire;
                    }
...
bool BanManager::Unban(char * sWhat) {
	...
	BanItem * pBan = FindNick(hash, acc_time, sWhat);

    if(pBan == NULL) {
		uint8_t ui128Hash[16];
		memset(ui128Hash, 0, 16);

		if(HashIP(sWhat, ui128Hash) == true && (pBan = FindIP(ui128Hash, acc_time)) != NULL) {
            Rem(pBan);
            delete pBan;
		} else {
            return false;
...
bool BanManager::PermUnban(char * sWhat) {
	...
	BanItem * pBan = FindPermNick(hash, sWhat);

    if(pBan == NULL) {
		uint8_t ui128Hash[16];
		memset(ui128Hash, 0, 16);

		if(HashIP(sWhat, ui128Hash) == true && (pBan = FindPermIP(ui128Hash)) != NULL) {
            Rem(pBan);
            delete pBan;
		} else {
            return false;
...
bool BanManager::TempUnban(char * sWhat) {
    ...
    BanItem * pBan = FindTempNick(hash, acc_time, sWhat);

    if(pBan == NULL) {
		uint8_t ui128Hash[16];
		memset(ui128Hash, 0, 16);

        if(HashIP(sWhat, ui128Hash) == true && (pBan = FindTempIP(ui128Hash, acc_time)) != NULL) {
            Rem(pBan);
            delete pBan;
		} else {
            return false;
