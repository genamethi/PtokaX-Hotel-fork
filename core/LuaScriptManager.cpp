...
void ScriptManager::LoadXML() {
  ...
  if (doc.LoadFile() == false) {
    if (doc.ErrorId() != TiXmlBase::TIXML_ERROR_OPENING_FILE &&
        doc.ErrorId() != TiXmlBase::TIXML_ERROR_DOCUMENT_EMPTY) {
      int iMsgLen = snprintf(
          ServerManager::m_pGlobalBuffer, ServerManager::m_szGlobalBufferSize,
          "Error loading file Scripts.xml. %s (Col: %d, Row: %d)",
          doc.ErrorDesc(), doc.Column(), doc.Row());
      if (iMsgLen > 0) {
#ifdef _BUILD_GUI
        ::MessageBox(NULL, ServerManager::m_pGlobalBuffer, g_sPtokaXTitle,
                     MB_OK | MB_ICONERROR);
#else
    ...
    if (scripts != NULL) {
      ...
      while ((child = scripts->IterateChildren(child)) != NULL) {
        ...
        if (FileExist((ServerManager::m_sScriptPath + string(name)).c_str()) ==
            false) {
          continue;
        }
...
ScriptManager::ScriptManager()
    : m_pRunningScriptE(NULL), m_pRunningScriptS(NULL), m_ppScriptTable(NULL),
      m_pActualUser(NULL), m_pTimerListS(NULL), m_pTimerListE(NULL),
      m_ui8ScriptCount(0), m_ui8BotsCount(0), m_bMoved(false) {
#ifdef _WIN32
...
#ifdef _WIN32
  FILE *fScriptsFile =
      fopen((ServerManager::m_sPath + "\\cfg\\Scripts.pxt").c_str(), "rt");
#else
  FILE *fScriptsFile =
      fopen((ServerManager::m_sPath + "/cfg/Scripts.pxt").c_str(), "rt");
#endif
  if (fScriptsFile == NULL) {
    int iMsgLen = snprintf(ServerManager::m_pGlobalBuffer,
                           ...
                           "Error loading file Scripts.pxt %s (%d)",
#ifdef _WIN32
  ...
  while (fgets(ServerManager::m_pGlobalBuffer,
               (int)ServerManager::m_szGlobalBufferSize,
               fScriptsFile) != NULL) {
    if (ServerManager::m_pGlobalBuffer[0] == '#' ||
        ServerManager::m_pGlobalBuffer[0] == '\n') {
      continue;
    }
    ...
    for (size_t szi = szLen - 1; szi != 0; szi--) {
      if (isspace(ServerManager::m_pGlobalBuffer[szi - 1]) != 0 ||
          ServerManager::m_pGlobalBuffer[szi - 1] == '=') {
    ...
    if (ServerManager::m_pGlobalBuffer[0] == '\0' ||
        (ServerManager::m_pGlobalBuffer[szLen - 1] != '1' &&
         ServerManager::m_pGlobalBuffer[szLen - 1] != '0')) {
      continue;
    }

    if (FileExist((ServerManager::m_sScriptPath +
                   string(ServerManager::m_pGlobalBuffer))
                      .c_str()) == false ||
        FindScript(ServerManager::m_pGlobalBuffer) != NULL) {
      continue;
    }

    AddScript(ServerManager::m_pGlobalBuffer,
              ServerManager::m_pGlobalBuffer[szLen - 1] == '1' ? true : false,
              false);
  }
...
ScriptManager::~ScriptManager() {
  ...
  if (HeapFree(ServerManager::m_hPtokaXHeap, HEAP_NO_SERIALIZE,
               (void *)m_ppScriptTable) == 0) {
    AppendDebugLog("%s - [MEM] Cannot deallocate m_ppScriptTable in "
                   "ScriptManager::~ScriptManager\n");
  }
#else
...
#ifdef _BUILD_GUI
bool ScriptManager::AddScript(char *sName, const bool bEnabled,
                              const bool bNew) {
#else
bool ScriptManager::AddScript(char *sName, const bool bEnabled,
                              const bool /*bNew*/) {
  ...
  if (m_ppScriptTable == NULL) {
    m_ppScriptTable = (Script **)HeapAlloc(
        ServerManager::m_hPtokaXHeap, HEAP_NO_SERIALIZE | HEAP_ZERO_MEMORY,
        (m_ui8ScriptCount + 1) * sizeof(Script *));
  } else {
    m_ppScriptTable = (Script **)HeapReAlloc(
        ServerManager::m_hPtokaXHeap, HEAP_NO_SERIALIZE, (void *)oldbuf,
        (m_ui8ScriptCount + 1) * sizeof(Script *));
  }
#else
  m_ppScriptTable =
      (Script **)realloc(oldbuf, (m_ui8ScriptCount + 1) * sizeof(Script *));
#endif
  if (m_ppScriptTable == NULL) {
    m_ppScriptTable = oldbuf;

    AppendDebugLog("%s - [MEM] Cannot (re)allocate m_ppScriptTable in "
  ...
  if (m_ppScriptTable[m_ui8ScriptCount] == NULL) {
    AppendDebugLog(
        "%s - [MEM] Cannot allocate new Script in ScriptManager::AddScript\n");
...
#ifdef _BUILD_GUI
  if (bNew == true) {
    MainWindowPageScripts::m_Ptr->ScriptToList(m_ui8ScriptCount - 1, true,
                                               false);
  }
#endif
...
void ScriptManager::Stop() {
  Script *S = NULL, *next = m_pRunningScriptS;
...
void ScriptManager::SaveScripts() {
  ...
  for (uint8_t ui8i = 0; ui8i < m_ui8ScriptCount; ui8i++) {
    if (FileExist((ServerManager::m_sScriptPath +
                   string(m_ppScriptTable[ui8i]->m_sName))
                      .c_str()) == false) {
      continue;
    }

    fprintf(fScriptsFile, "%s\t=\t%c\n", m_ppScriptTable[ui8i]->m_sName,
            m_ppScriptTable[ui8i]->m_bEnabled == true ? '1' : '0');
  }
...
void ScriptManager::CheckForDeletedScripts() {
  ...
  while (ui8i < m_ui8ScriptCount) {
    if (FileExist((ServerManager::m_sScriptPath +
                   string(m_ppScriptTable[ui8i]->m_sName))
                      .c_str()) == true ||
        m_ppScriptTable[ui8i]->m_pLua != NULL) {
      ui8i++;
      continue;
    }
...
void ScriptManager::CheckForNewScripts() {
  ...
  intptr_t hFile =
      _findfirst((ServerManager::m_sScriptPath + "\\*.lua").c_str(), &luafile);
  ...
  while ((p_dirent = readdir(p_scriptdir)) != NULL) {
    if (stat((ServerManager::m_sScriptPath + p_dirent->d_name).c_str(),
             &s_buf) != 0 ||
        (s_buf.st_mode & S_IFDIR) != 0 ||
        strcasecmp(p_dirent->d_name + (strlen(p_dirent->d_name) - 4), ".lua") !=
            0) {
      continue;
    }
...
Script *ScriptManager::FindScript(lua_State *pLua) {
  Script *cur = NULL, *next = m_pRunningScriptS;
...
void ScriptManager::MoveScript(const uint8_t ui8ScriptPosInTbl,
                               const bool bUp) {
  if (bUp == true) {
    ...
    if (pScript->m_pLua == NULL ||
        m_ppScriptTable[ui8ScriptPosInTbl]->m_pLua == NULL) {
      return;
    }
...
void ScriptManager::DeleteScript(const uint8_t ui8ScriptPosInTbl) {
  ...
  if (FileExist(
          (ServerManager::m_sScriptPath + string(pScript->m_sName)).c_str()) ==
      true) {
#ifdef _WIN32
    DeleteFile(
        (ServerManager::m_sScriptPath + string(pScript->m_sName)).c_str());
...
void ScriptManager::OnStartup() {
  ...
  Script *pScript = NULL, *next = m_pRunningScriptS;
  ...
  while (next != NULL) {
    ...
    if (pScript->m_pLua != NULL &&
        ((pScript->m_ui16Functions & Script::ONSTARTUP) == Script::ONSTARTUP) ==
            true &&
        (m_bMoved == false || pScript->m_bProcessed == false)) {
...
void ScriptManager::OnExit(const bool bForce /* = false*/) {
  ...
  while (next != NULL) {
    ...
    if (pScript->m_pLua != NULL &&
        ((pScript->m_ui16Functions & Script::ONEXIT) == Script::ONEXIT) ==
            true &&
        (m_bMoved == false || pScript->m_bProcessed == false)) {
      pScript->m_bProcessed = true;
      ScriptOnExit(pScript);
...
//------------------------------------------------------------------------------
// Study the function below closely, a few things to consider here.
bool ScriptManager::Arrival(DcCommand *pDcCommand, const uint8_t ui8Type) {
  ...
  static const uint32_t iLuaArrivalBits[] = {
      ...
      0x80,   0x100,  0x200,   0x400,   0x800,   0x1000,  0x2000,
  ...
  while (next != NULL) {
    ...
    if (((cur->m_ui32DataArrivals & iLuaArrivalBits[ui8Type]) ==
         iLuaArrivalBits[ui8Type]) == true &&
        (m_bMoved == false || cur->m_bProcessed == false)) {
      ...
      static const char *arrival[] = {
          "ChatArrival",           "KeyArrival",
          "ValidateNickArrival",   "PasswordArrival",
          "VersionArrival",        "GetNickListArrival",
          "MyINFOArrival",         "GetINFOArrival",
          "SearchArrival",         "ToArrival",
          "ConnectToMeArrival",    "MultiConnectToMeArrival",
          "RevConnectToMeArrival", "SRArrival",
          "UDPSRArrival",          "KickArrival",
          "OpForceMoveArrival",    "SupportsArrival",
          "BotINFOArrival",        "CloseArrival",
          "UnknownArrival"};
      ...
      lua_checkstack(cur->m_pLua,
                     2); // we need 2 empty slots in stack, check it to be sure

      ScriptPushUser(cur->m_pLua, pDcCommand->m_pUser); // usertable
      lua_pushlstring(cur->m_pLua, pDcCommand->m_sCommand,
                      pDcCommand->m_ui32CommandLen); // sData
      ...
      if (lua_type(cur->m_pLua, iTop) != LUA_TBOOLEAN ||
          lua_toboolean(cur->m_pLua, iTop) == 0) {
        lua_settop(cur->m_pLua, 0);
        continue;
      }
...
bool ScriptManager::UserConnected(User *pUser) {
  ...
  while (next != NULL) {
    ...
    static const uint32_t iConnectedBits[] = {
        Script::USERCONNECTED, Script::REGCONNECTED, Script::OPCONNECTED};

    if (((cur->m_ui16Functions & iConnectedBits[ui8Type]) ==
         iConnectedBits[ui8Type]) == true &&
        (m_bMoved == false || cur->m_bProcessed == false)) {
      cur->m_bProcessed = true;
      ...
      static const char *ConnectedFunction[] = {"UserConnected", "RegConnected",
                                                "OpConnected"};
      ...
      lua_checkstack(cur->m_pLua,
                     1); // we need 1 empty slots in stack, check it to be sure
...
void ScriptManager::UserDisconnected(User *pUser, Script *pScript /* = NULL*/) {
  ...
  while (next != NULL) {
    ...
    static const uint32_t iDisconnectedBits[] = {Script::USERDISCONNECTED,
                                                 Script::REGDISCONNECTED,
                                                 Script::OPDISCONNECTED};

    if (((cur->m_ui16Functions & iDisconnectedBits[ui8Type]) ==
         iDisconnectedBits[ui8Type]) == true &&
        (m_bMoved == false || cur->m_bProcessed == false)) {
      cur->m_bProcessed = true;
      ...
      static const char *DisconnectedFunction[] = {
