...
unsigned __stdcall ExecuteLoop(void *) {
  ...
  while (true) {
    dwRet = ::WaitForMultipleObjects(2, ServiceLoop::m_hLoopEvents, FALSE,
                                     INFINITE);
...
ServiceLoop::AcceptedSocket::AcceptedSocket()
    : m_pNext(NULL), m_bProxy(false),
#ifdef _WIN32
...
void ServiceLoop::Looper() {
  // PPK ... two loop strategy for saving bandwith
  if (m_bRecv == true) {
    ReceiveLoop();
  } else {
    SendLoop();
  ...
  if (ServerManager::m_bServerTerminated == false) {
    ...
    if (::SetEvent(m_hLoopEvents[0]) == 0) {
      AppendDebugLog(
          "%s - [ERR] Cannot set m_hLoopEvent in ServiceLoop::Looper\n");
      exit(EXIT_FAILURE);
    }
#endif
  } else {
...
ServiceLoop::ServiceLoop()
    : m_ui64LstUptmTck(ServerManager::m_ui64ActualTick),
      m_pAcceptedSocketsS(NULL), m_pAcceptedSocketsE(NULL), m_dLoggedUsers(0),
      m_dActualSrvLoopLogins(0), m_ui32LastSendRest(0), m_ui32SendRestsPeak(0),
      m_ui32LastRecvRest(0), m_ui32RecvRestsPeak(0), m_ui32LoopsForLogins(0),
      m_bRecv(true) {
#ifdef _WIN32
  ...
  if (m_hLoopEvents[0] == NULL) {
    AppendDebugLog(
        "%s - [ERR] Cannot create m_hLoopEvent in ServiceLoop::ServiceLoop\n");
    exit(EXIT_FAILURE);
  }
  ...
  if (m_hLoopEvents[1] == NULL) {
    AppendDebugLog("%s - [ERR] Cannot create m_hTerminateEvent in "
                   "ServiceLoop::ServiceLoop\n");
    exit(EXIT_FAILURE);
  }

  m_hThreadHandle =
      (HANDLE)::_beginthreadex(NULL, 0, ExecuteLoop, NULL, 0, NULL);
  if (m_hThreadHandle == 0) {
    AppendDebugLog("%s - [ERR] Failed to create hThreadHandle in "
                   "ServiceLoop::ServiceLoop\n");
    exit(EXIT_FAILURE);
  }
#endif
...
ServiceLoop::~ServiceLoop() {
  AcceptedSocket *cursck = NULL, *nextsck = m_pAcceptedSocketsS;
...
void ServiceLoop::DerivePeerAddress(const sockaddr_storage &Addr, char *sIP,
                                    uint8_t *ui128IpHash,
                                    uint16_t &ui16IpTableIdx, bool &bIPv6) {
  if (Addr.ss_family == AF_INET6) {
    memcpy(ui128IpHash, &((struct sockaddr_in6 *)&Addr)->sin6_addr.s6_addr, 16);

    if (IN6_IS_ADDR_V4MAPPED(&((struct sockaddr_in6 *)&Addr)->sin6_addr)) {
      in_addr ipv4addr;
      memcpy(&ipv4addr, ((struct sockaddr_in6 *)&Addr)->sin6_addr.s6_addr + 12,
             4);
    ...
    ui128IpHash[10] = 255;
    ui128IpHash[11] = 255;
    memcpy(ui128IpHash + 12, &((struct sockaddr_in *)&Addr)->sin_addr.s_addr,
           4);
...
// Second half of AcceptUser for a proxied connection, run once the header names
// the real address. The direct path does all of this before the User exists,
// which keeps a banned address from costing an allocation. Here the User is
// already there, so a full ban is delivered through its own send buffer instead
// of the raw socket.
bool ServiceLoop::ApplyPeerAddress(User *pUser, const sockaddr_storage &Addr) {
  ...
  if (Ban != NULL) {
    ...
    pUser->m_pLogInOut->m_pBan =
        UserBan::CreateUserBan(ServerManager::m_pGlobalBuffer, iMsgLen, hash);

    if (pUser->m_pLogInOut->m_pBan == NULL) {
      AppendDebugLog("%s - [MEM] Cannot allocate new uBan in "
                     "ServiceLoop::ApplyPeerAddress\n");
      return false;
    }
  ...
  RangeBanItem *RangeBan =
      BanManager::m_Ptr->FindFullRange(ui128IpHash, acc_time);
  ...
  if (RangeBan != NULL) {
    ...
    pUser->m_pLogInOut->m_pBan =
        UserBan::CreateUserBan(ServerManager::m_pGlobalBuffer, iMsgLen, 0);

    if (pUser->m_pLogInOut->m_pBan == NULL) {
      AppendDebugLog("%s - [MEM] Cannot allocate new uBan in "
                     "ServiceLoop::ApplyPeerAddress1\n");
      return false;
    }
  }
...
void ServiceLoop::AcceptUser(AcceptedSocket *pAccptSocket) {
  ...
  DerivePeerAddress(pAccptSocket->m_Addr, sIP, ui128IpHash, ui16IpTableIdx,
                    bIPv6);

  // set the recv buffer
#ifdef _WIN32
  int32_t bufsize = 8192;
  if (setsockopt(pAccptSocket->m_Socket, SOL_SOCKET, SO_RCVBUF,
                 (char *)&bufsize, sizeof(bufsize)) == SOCKET_ERROR) {
    int iError = WSAGetLastError();
    UdpDebug::m_Ptr->BroadcastFormat("[SYS] setsockopt failed on attempt to "
                                     "set SO_RCVBUF. IP: %s Err: %s (%d)",
                                     sIP, WSErrorStr(iError), iError);
#else
  int bufsize = 8192;
  if (setsockopt(pAccptSocket->m_Socket, SOL_SOCKET, SO_RCVBUF, &bufsize,
                 sizeof(bufsize)) == -1) {
    UdpDebug::m_Ptr->BroadcastFormat("[SYS] setsockopt failed on attempt to "
                                     "set SO_RCVBUF. IP: %s Err: %s (%d)",
                                     sIP, ErrnoStr(errno), errno);
#endif
#ifdef _WIN32
  ...
  bufsize = 32768;
#ifdef _WIN32
  if (setsockopt(pAccptSocket->m_Socket, SOL_SOCKET, SO_SNDBUF,
                 (char *)&bufsize, sizeof(bufsize)) == SOCKET_ERROR) {
    int iError = WSAGetLastError();
    UdpDebug::m_Ptr->BroadcastFormat("[SYS] setsockopt failed on attempt to "
                                     "set SO_SNDBUF. IP: %s Err: %s (%d)",
                                     sIP, WSErrorStr(iError), iError);
#else
  if (setsockopt(pAccptSocket->m_Socket, SOL_SOCKET, SO_SNDBUF, &bufsize,
                 sizeof(bufsize)) == -1) {
    UdpDebug::m_Ptr->BroadcastFormat("[SYS] setsockopt failed on attempt to "
                                     "set SO_SNDBUF. IP: %s Err: %s (%d)",
                                     sIP, ErrnoStr(errno), errno);
  ...
  setsockopt(pAccptSocket->m_Socket, SOL_SOCKET, SO_KEEPALIVE,
             (char *)&bKeepalive, sizeof(bKeepalive));
#else
  int iKeepAlive = 1;
  if (setsockopt(pAccptSocket->m_Socket, SOL_SOCKET, SO_KEEPALIVE, &iKeepAlive,
                 sizeof(iKeepAlive)) == -1) {
    UdpDebug::m_Ptr->BroadcastFormat("[SYS] setsockopt failed on attempt to "
                                     "set SO_KEEPALIVE. IP: %s Err: %s (%d)",
                                     sIP, ErrnoStr(errno), errno);
...
#ifdef _WIN32
  uint32_t block = 1;
  if (ioctlsocket(pAccptSocket->m_Socket, FIONBIO, (unsigned long *)&block) ==
      SOCKET_ERROR) {
    int iError = WSAGetLastError();
    UdpDebug::m_Ptr->BroadcastFormat("[SYS] ioctlsocket failed on attempt to "
                                     "set FIONBIO. IP: %s Err: %s (%d)",
                                     sIP, WSErrorStr(iError), iError);
#else
  ...
  if (fcntl(pAccptSocket->m_Socket, F_SETFL, oldFlag | O_NONBLOCK) == -1) {
    UdpDebug::m_Ptr->BroadcastFormat(
        "[SYS] fcntl failed on attempt to set O_NONBLOCK. IP: %s Err: %s (%d)",
        sIP, ErrnoStr(errno), errno);
#endif
#ifdef _WIN32
    shutdown(pAccptSocket->m_Socket, SD_SEND);
  ...
  if (SettingManager::m_Ptr->m_bBools[SETBOOL_REDIRECT_ALL] == true) {
    if (SettingManager::m_Ptr->m_sTexts[SETTXT_REDIRECT_ADDRESS] != NULL) {
      int iMsgLen = snprintf(
          ...
          LanguageManager::m_Ptr->m_sTexts[LAN_YOU_REDIR_TO],
          SettingManager::m_Ptr->m_sTexts[SETTXT_REDIRECT_ADDRESS],
          SettingManager::m_Ptr
              ->m_sPreTexts[SettingManager::SETPRETXT_REDIRECT_ADDRESS]);
      if (iMsgLen > 0) {
        send(pAccptSocket->m_Socket, ServerManager::m_pGlobalBuffer, iMsgLen,
             0);
  ...
  // a proxied connection carries the proxy's address at this point, so the ban
  // lookups wait for the header and run in ApplyPeerAddress instead
  BanItem *Ban = pAccptSocket->m_bProxy == true
                     ? NULL
                     : BanManager::m_Ptr->FindFull(ui128IpHash, acc_time);
  ...
  if (Ban != NULL) {
    if (((Ban->m_ui8Bits & BanManager::FULL) == BanManager::FULL) == true) {
      ...
      close(pAccptSocket->m_Socket);
#endif
      //			UdpDebug::m_Ptr->BroadcastFormat("[SYS] Banned
      //ip %s - connection closed.", sIP);
  ...
  if (RangeBan != NULL) {
    if (((RangeBan->m_ui8Bits & BanManager::FULL) == BanManager::FULL) ==
      ...
      close(pAccptSocket->m_Socket);
#endif
      //            UdpDebug::m_Ptr->BroadcastFormat("[SYS] Range Banned ip %s -
      //            connection closed.", sIP);
...
#endif
    AppendDebugLog(
        "%s - [MEM] Cannot allocate pUser in ServiceLoop::AcceptUser\n");
    return;
  }
  ...
  if (pUser->m_pLogInOut == NULL) {
    ...
    AppendDebugLog(
        "%s - [MEM] Cannot allocate pLogInOut in ServiceLoop::AcceptUser\n");
    return;
  }
  ...
  if (Ban != NULL) {
    ...
    if (pUser->m_pLogInOut->m_pBan == NULL) {
      ...
      AppendDebugLog(
          "%s - [MEM] Cannot allocate new uBan in ServiceLoop::AcceptUser\n");
    ...
    if (pUser->m_pLogInOut->m_pBan == NULL) {
      ...
      AppendDebugLog(
          "%s - [MEM] Cannot allocate new uBan in ServiceLoop::AcceptUser1\n");
  ...
  // The address above is the proxy's. Hold the user short of the $Lock until
  // the header arrives, so ban checks and SetIP run against the client's own
  // address.
...
void ServiceLoop::ReceiveLoop() {
  ...
  if (SettingManager::m_Ptr->m_bBools[SETBOOL_AUTO_REG] == true) {
    if (((uint64_t)ts.tv_sec - m_ui64LastRegToHublist) >=
        901) { // 15 min 1 sec is hublist register interval
      m_ui64LastRegToHublist = ts.tv_sec;
  ...
  ScriptOnTimer((uint64_t(ts.tv_sec) * 1000) +
  ...
  if (SettingManager::m_Ptr->m_bBools[SETBOOL_AUTO_REG] == true) {
    if ((ui64Secs - ui64LastRegToHublist) >=
        901) { // 15 min 1 sec is hublist register interval
  ...
  if (ServerManager::m_ui8SrCntr >= 7 ||
      (Users::m_Ptr->m_ui16ActSearchs + Users::m_Ptr->m_ui16PasSearchs) > 8 ||
  ...
  if (ServerManager::m_ui64ActualTick - m_ui64LstUptmTck > 60) {
    ...
    if (ServerManager::m_ui64Mins == 0 || ServerManager::m_ui64Mins == 15 ||
        ServerManager::m_ui64Mins == 30 || ServerManager::m_ui64Mins == 45) {
      ...
      if (HeapValidate(ServerManager::m_hPtokaXHeap, HEAP_NO_SERIALIZE, 0) ==
          0) {
  ...
  AcceptedSocket *CurSck = NULL, *NextSck = NULL;
  ...
  User *curUser = NULL, *nextUser = Users::m_Ptr->m_pUserListS;
  ...
  while (nextUser != 0 && ServerManager::m_bServerTerminated == false) {
    ...
    if (curUser->DoRecv() == true) {
      iRecvRests++;
      // Memo("Rest " + string(curUser->Nick, curUser->NickLen) + ": '" +
      // string(curUser->pRecvBuf) + "'");
    }
    ...
    case User::STATE_PROXY_HEADER: {
      ...
      const PxProxyResult eResult = PxProxyParse(
          curUser->m_pRecvBuf, curUser->m_ui32RecvBufDataLen, Header);

      if (eResult == PX_PROXY_NEED_MORE) {
        // same 20 second budget the login states get
        if (ServerManager::m_ui64ActualTick -
                curUser->m_pLogInOut->m_ui64LogonTick >
            20) {
          UdpDebug::m_Ptr->BroadcastFormat(
              "[SYS] Proxy header timeout for %s - connection closed.",
              curUser->m_sIP);
          curUser->Close();
        }
      ...
      if (eResult != PX_PROXY_OK) {
        UdpDebug::m_Ptr->BroadcastFormat(
            "[SYS] Bad proxy header from %s - connection closed.",
            curUser->m_sIP);
        curUser->Close();
        continue;
      }
      ...
      if (Header.m_bSecure == true) {
        ...
        memcpy(curUser->m_sTLSVersion, Header.m_sTLSVersion,
               sizeof(curUser->m_sTLSVersion));
      }
      ...
      if (curUser->m_ui32RecvBufDataLen != 0) {
        memmove(curUser->m_pRecvBuf, curUser->m_pRecvBuf + Header.m_szConsumed,
                curUser->m_ui32RecvBufDataLen);
      }
    ...
    case User::STATE_KEY_OR_SUP: {
      ...
      if (ServerManager::m_ui64ActualTick -
          ...
          20) {
        UdpDebug::m_Ptr->BroadcastFormat(
            "[SYS] Login timeout 1 for %s - user disconnected.",
            curUser->m_sIP);
    ...
    case User::STATE_IPV4_CHECK: {
      ...
      if ((ServerManager::m_ui64ActualTick -
           curUser->m_pLogInOut->m_ui64IPv4CheckTick) > 10) {
        UdpDebug::m_Ptr->BroadcastFormat("[SYS] IPv4Check timeout for %s (%s).",
                                         curUser->m_sNick, curUser->m_sIP);
    ...
    case User::STATE_ADDME: {
      // PPK ... Add user, but only if send $GetNickList (or have quicklist
      // supports) <- important, used by flooders !!!
      if (((curUser->m_ui32BoolBits & User::BIT_GETNICKLIST) ==
           User::BIT_GETNICKLIST) == false &&
          ((curUser->m_ui32SupportBits & User::SUPPORTBIT_QUICKLIST) ==
           User::SUPPORTBIT_QUICKLIST) == false &&
          ((curUser->m_ui32BoolBits & User::BIT_PINGER) == User::BIT_PINGER) ==
              true)
        continue;

      curUser->SendFormat(
          "ServiceLoop::ReceiveLoop->User::STATE_ADDME", true,
    ...
    case User::STATE_ADDME_1LOOP: {
      ...
      if (m_dLoggedUsers >= m_dActualSrvLoopLogins &&
          ((curUser->m_ui32BoolBits & User::BIT_OPERATOR) ==
           User::BIT_OPERATOR) == false) {
        if (ServerManager::m_ui64ActualTick -
                curUser->m_pLogInOut->m_ui64LogonTick >
            300) {
          UdpDebug::m_Ptr->BroadcastFormat(
              "[SYS] Login timeout (%d) 3 for %s (%s) - user disconnected.",
              (int)curUser->m_ui8State, curUser->m_sNick, curUser->m_sIP);
      ...
      if ((curUser->m_ui32BoolBits & User::BIT_IPV6) == User::BIT_IPV6 &&
          ((curUser->m_ui32BoolBits & User::SUPPORTBIT_IPV4) ==
           User::SUPPORTBIT_IPV4) == false) {
        in_addr ipv4addr;
        ipv4addr.s_addr = INADDR_NONE;

        if (curUser->m_ui128IpHash[0] == 32 &&
            curUser->m_ui128IpHash[1] == 2) { // 6to4 tunnel
          memcpy(&ipv4addr, curUser->m_ui128IpHash + 2, 4);
        } else if (curUser->m_ui128IpHash[0] == 32 &&
                   curUser->m_ui128IpHash[1] == 1 &&
                   curUser->m_ui128IpHash[2] == 0 &&
      ...
      // New User Connected ... the user is operator ? invoke lua
      // User/OpConnected
      uint32_t iBeforeLuaLen = curUser->m_ui32SendBufDataLen;

      bool bRet = ScriptManager::m_Ptr->UserConnected(curUser);
      if (curUser->m_ui8State >=
          User::STATE_CLOSING) { // connection closed by script?
      ...
      if (iBeforeLuaLen < curUser->m_ui32SendBufDataLen) {
        ...
        if (curUser->m_pLogInOut->m_pBuffer == NULL) {
          curUser->m_pLogInOut->m_pBuffer = (char *)HeapAlloc(
              ServerManager::m_hPtokaXHeap, HEAP_NO_SERIALIZE, szNeededLen + 1);
        } else {
          curUser->m_pLogInOut->m_pBuffer =
              (char *)HeapReAlloc(ServerManager::m_hPtokaXHeap,
                                  HEAP_NO_SERIALIZE, sOldBuf, szNeededLen + 1);
        }
#else
        curUser->m_pLogInOut->m_pBuffer =
            (char *)realloc(sOldBuf, szNeededLen + 1);
#endif
        if (curUser->m_pLogInOut->m_pBuffer == NULL) {
          curUser->m_ui32BoolBits |= User::BIT_ERROR;
          curUser->Close();

          AppendDebugLogFormat("[MEM] Cannot allocate %zu bytes for "
                               "sLockUsrConn in ServiceLoop::ReceiveLoop\n",
                               szNeededLen + 1);

          continue;
        }
        memcpy(curUser->m_pLogInOut->m_pBuffer,
               curUser->m_pSendBuf + iBeforeLuaLen, szNeededLen);
        curUser->m_pLogInOut->m_ui32UserConnectedLen = (uint32_t)szNeededLen;
        curUser->m_pLogInOut
            ->m_pBuffer[curUser->m_pLogInOut->m_ui32UserConnectedLen] = '\0';
      ...
      if (((curUser->m_ui32BoolBits & User::BIT_HAVE_BADTAG) ==
...
#ifdef _BUILD_GUI
      if (::SendMessage(MainWindowPageUsersChat::m_Ptr->m_hWndPageItems
                            [MainWindowPageUsersChat::BTN_AUTO_UPDATE_USERLIST],
...
//                if(sqldb) sqldb->AddVisit(curUser);
      ...
      int iMsgLen = snprintf(ServerManager::m_pGlobalBuffer,
                             ServerManager::m_szGlobalBufferSize, "$Hello %s|",
                             curUser->m_sNick);
      if (iMsgLen > 0) {
        GlobalDataQueue::m_Ptr->AddQueueItem(ServerManager::m_pGlobalBuffer,
                                             iMsgLen, NULL, 0,
                                             GlobalDataQueue::CMD_HELLO);
      }
      ...
      case 0:
        GlobalDataQueue::m_Ptr->AddQueueItem(curUser->m_sMyInfoLong,
                                             curUser->m_ui16MyInfoLongLen, NULL,
                                             0, GlobalDataQueue::CMD_MYINFO);
        break;
      case 1:
        GlobalDataQueue::m_Ptr->AddQueueItem(
            curUser->m_sMyInfoShort, curUser->m_ui16MyInfoShortLen,
            curUser->m_sMyInfoLong, curUser->m_ui16MyInfoLongLen,
            GlobalDataQueue::CMD_MYINFO);
        break;
      case 2:
        GlobalDataQueue::m_Ptr->AddQueueItem(
            curUser->m_sMyInfoShort, curUser->m_ui16MyInfoShortLen, NULL, 0,
            GlobalDataQueue::CMD_MYINFO);
        break;
      default:
        break;
      ...
      if (((curUser->m_ui32BoolBits & User::BIT_OPERATOR) ==
    ...
    case User::STATE_ADDED:
      if (curUser->m_pCmdToUserStrt != NULL) {
        PrcsdToUsrCmd *cur = NULL, *next = curUser->m_pCmdToUserStrt;
        ...
        while (next != NULL) {
                ...
                if (cur->m_pToUser->m_ui32ReceivedPmCount == 0) {
                  ...
                  if (cur->m_pToUser->m_ui64ReceivedPmTick + 60 <
                    ...
                    if (cur->m_ui32PmCount == 1) {
                      curUser->SendFormat(
                          "ServiceLoop::ReceiveLoop->User::STATE_ADDED1", true,
                          "$To: %s From: %s $<%s> %s %s %s!|", curUser->m_sNick,
                          cur->m_sToNick,
                          SettingManager::m_Ptr
                              ->m_sPreTexts[SettingManager::SETPRETXT_HUB_SEC],
                          LanguageManager::m_Ptr->m_sTexts[LAN_SRY_LST_MSG_BCS],
                          cur->m_sToNick,
                          LanguageManager::m_Ptr->m_sTexts[LAN_EXC_MSG_LIMIT]);
                    } else {
                      curUser->SendFormat(
                          "ServiceLoop::ReceiveLoop->User::STATE_ADDED2", true,
                          "$To: %s From: %s $<%s> %s %u %s %s %s!|",
                          curUser->m_sNick, cur->m_sToNick,
                          SettingManager::m_Ptr
                              ->m_sPreTexts[SettingManager::SETPRETXT_HUB_SEC],
                          LanguageManager::m_Ptr->m_sTexts[LAN_SORRY_LAST],
                          cur->m_ui32PmCount,
                          LanguageManager::m_Ptr->m_sTexts[LAN_MSGS_NOT_SENT],
                          cur->m_sToNick,
                          LanguageManager::m_Ptr->m_sTexts[LAN_EXC_MSG_LIMIT]);
                    }
#ifdef _WIN32
                    if (HeapFree(ServerManager::m_hPtokaXHeap,
                                 HEAP_NO_SERIALIZE,
                                 (void *)cur->m_sCommand) == 0) {
                      AppendDebugLog(
                          "%s - [MEM] Cannot deallocate cur->sCommand in "
                          "ServiceLoop::ReceiveLoop\n");
                    }
#else
                    free(cur->m_sCommand);
#endif
                    ...
                    if (HeapFree(ServerManager::m_hPtokaXHeap,
                      ...
                      AppendDebugLog(
                          "%s - [MEM] Cannot deallocate cur->m_sToNick in "
                          "ServiceLoop::ReceiveLoop\n");
                    }
#else
                    free(cur->m_sToNick);
          ...
          if (HeapFree(ServerManager::m_hPtokaXHeap, HEAP_NO_SERIALIZE,
                       (void *)cur->m_sToNick) == 0) {
            AppendDebugLog("%s - [MEM] Cannot deallocate cur->m_ToNick in "
                           "ServiceLoop::ReceiveLoop\n");
          }
#else
          free(cur->m_sToNick);
      ...
      if (ServerManager::m_ui8SrCntr == 0) {
        if (curUser->m_pCmdActive6Search != NULL) {
          if (curUser->m_pCmdActive4Search != NULL) {
            GlobalDataQueue::m_Ptr->AddQueueItem(
                curUser->m_pCmdActive6Search->m_sCommand,
                curUser->m_pCmdActive6Search->m_ui32Len,
                curUser->m_pCmdActive4Search->m_sCommand,
                curUser->m_pCmdActive4Search->m_ui32Len,
                GlobalDataQueue::CMD_ACTIVE_SEARCH_V64);
          } else {
            GlobalDataQueue::m_Ptr->AddQueueItem(
                curUser->m_pCmdActive6Search->m_sCommand,
                curUser->m_pCmdActive6Search->m_ui32Len, NULL, 0,
                GlobalDataQueue::CMD_ACTIVE_SEARCH_V6);
          }
        } else if (curUser->m_pCmdActive4Search != NULL) {
          GlobalDataQueue::m_Ptr->AddQueueItem(
              curUser->m_pCmdActive4Search->m_sCommand,
              curUser->m_pCmdActive4Search->m_ui32Len, NULL, 0,
              GlobalDataQueue::CMD_ACTIVE_SEARCH_V4);
        }
        ...
        if (curUser->m_pCmdPassiveSearch != NULL) {
          ...
          if ((curUser->m_ui32BoolBits & User::BIT_IPV6) == User::BIT_IPV6) {
            if ((curUser->m_ui32BoolBits & User::BIT_IPV4) == User::BIT_IPV4) {
              if ((curUser->m_ui32BoolBits & User::BIT_IPV6_ACTIVE) ==
                  User::BIT_IPV6_ACTIVE) {
                ui8CmdType = GlobalDataQueue::CMD_PASSIVE_SEARCH_V4_ONLY;
              } else if ((curUser->m_ui32BoolBits & User::BIT_IPV4_ACTIVE) ==
          ...
          GlobalDataQueue::m_Ptr->AddQueueItem(
              curUser->m_pCmdPassiveSearch->m_sCommand,
              curUser->m_pCmdPassiveSearch->m_ui32Len, NULL, 0, ui8CmdType);
        ...
        if (curUser->m_sLastChat != NULL && curUser->m_ui16LastChatLines < 2 &&
            (curUser->m_ui64SameChatsTick +
             SettingManager::m_Ptr->m_i16Shorts[SETSHORT_SAME_MAIN_CHAT_TIME]) <
                ServerManager::m_ui64ActualTick) {
#ifdef _WIN32
          if (HeapFree(ServerManager::m_hPtokaXHeap, HEAP_NO_SERIALIZE,
                       (void *)curUser->m_sLastChat) == 0) {
            AppendDebugLog("%s - [MEM] Cannot deallocate curUser->m_sLastChat "
                           "in ServiceLoop::ReceiveLoop\n");
          }
#else
        ...
        if (curUser->m_sLastPM != NULL && curUser->m_ui16LastPmLines < 2 &&
            (curUser->m_ui64SamePMsTick +
             SettingManager::m_Ptr->m_i16Shorts[SETSHORT_SAME_PM_TIME]) <
                ServerManager::m_ui64ActualTick) {
#ifdef _WIN32
          if (HeapFree(ServerManager::m_hPtokaXHeap, HEAP_NO_SERIALIZE,
                       (void *)curUser->m_sLastPM) == 0) {
            AppendDebugLog("%s - [MEM] Cannot deallocate curUser->m_sLastPM in "
                           "ServiceLoop::ReceiveLoop\n");
          }
#else
        ...
        if (curUser->m_sLastSearch != NULL &&
            (curUser->m_ui64SameSearchsTick +
             SettingManager::m_Ptr->m_i16Shorts[SETSHORT_SAME_SEARCH_TIME]) <
                ServerManager::m_ui64ActualTick) {
#ifdef _WIN32
          if (HeapFree(ServerManager::m_hPtokaXHeap, HEAP_NO_SERIALIZE,
                       (void *)curUser->m_sLastSearch) == 0) {
            AppendDebugLog(
                "%s - [MEM] Cannot deallocate curUser->m_sLastSearch in "
                "ServiceLoop::ReceiveLoop\n");
          }
#else
    ...
    case User::STATE_CLOSING: {
      if (((curUser->m_ui32BoolBits & User::BIT_ERROR) == User::BIT_ERROR) ==
              false &&
          curUser->m_ui32SendBufDataLen != 0) {
        if (curUser->m_pLogInOut->m_ui32ToCloseLoops != 0 ||
            ((curUser->m_ui32BoolBits & User::BIT_PINGER) ==
    ...
    default: {
      ...
      if (ServerManager::m_ui64ActualTick -
          ...
          60) {
        UdpDebug::m_Ptr->BroadcastFormat(
            "[SYS] Login timeout (%d) 2 for %s (%s) - user disconnected.",
            (int)curUser->m_ui8State, curUser->m_sNick, curUser->m_sIP);
  ...
  m_ui32RecvRestsPeak =
      iRecvRests > m_ui32RecvRestsPeak ? iRecvRests : m_ui32RecvRestsPeak;
}
//---------------------------------------------------------------------------
...
void ServiceLoop::SendLoop() {
  ...
  // old users get it in this loop from queue -> badwith saving !!! no more
  // twice myinfo =) Sending Loop
  ...
  while (nextUser != 0 && ServerManager::m_bServerTerminated == false) {
    ...
    case User::STATE_ADDME_2LOOP: {
      ...
      if (ServerManager::m_ui32Peak < ServerManager::m_ui32Logged) {
        ...
        if (SettingManager::m_Ptr->m_i16Shorts[SETSHORT_MAX_USERS_PEAK] <
            (int16_t)ServerManager::m_ui32Peak)
          SettingManager::m_Ptr->SetShort(SETSHORT_MAX_USERS_PEAK,
                                          (int16_t)ServerManager::m_ui32Peak);
      }
      ...
      if (((curUser->m_ui32SupportBits & User::SUPPORTBIT_USERIP2) ==
           User::SUPPORTBIT_USERIP2) == true &&
          ((curUser->m_ui32BoolBits & User::BIT_QUACK_SUPPORTS) ==
           User::BIT_QUACK_SUPPORTS) == false &&
          ProfileManager::m_Ptr->IsAllowed(
              curUser, ProfileManager::SENDALLUSERIP) == false) {
        curUser->SendFormat(
            "ServiceLoop::SendLoop->User::STATE_ADDME_2LOOP1", true,
            "$UserIP %s %s|", curUser->m_sNick,
            (curUser->m_sIPv4[0] == '\0' ? curUser->m_sIP : curUser->m_sIPv4));
      }
      ...
      // PPK ... send motd ???
      if (SettingManager::m_Ptr
              ->m_ui16PreTextsLens[SettingManager::SETPRETXT_MOTD] != 0) {
        if (SettingManager::m_Ptr->m_bBools[SETBOOL_MOTD_AS_PM] == true) {
          curUser->SendFormat("ServiceLoop::SendLoop->User::STATE_ADDME_2LOOP2",
                              true,
                              SettingManager::m_Ptr
                                  ->m_sPreTexts[SettingManager::SETPRETXT_MOTD],
                              curUser->m_sNick);
        } else {
          curUser->SendCharDelayed(
              SettingManager::m_Ptr
                  ->m_sPreTexts[SettingManager::SETPRETXT_MOTD],
              SettingManager::m_Ptr
                  ->m_ui16PreTextsLens[SettingManager::SETPRETXT_MOTD]);
        }
      }
      ...
      if (curUser->m_pLogInOut->m_ui32UserConnectedLen != 0) {
        curUser->PutInSendBuf(curUser->m_pLogInOut->m_pBuffer,
                              curUser->m_pLogInOut->m_ui32UserConnectedLen);
  ...
  m_dActualSrvLoopLogins +=
      (double)
          SettingManager::m_Ptr->m_i16Shorts[SETSHORT_MAX_SIMULTANEOUS_LOGINS] /
      40;
  m_ui32LoopsForLogins++;

  m_ui32LastSendRest = iSendRests;
  m_ui32SendRestsPeak =
      iSendRests > m_ui32SendRestsPeak ? iSendRests : m_ui32SendRestsPeak;
}
//---------------------------------------------------------------------------

#ifdef _WIN32
void ServiceLoop::AcceptSocket(const SOCKET s, const sockaddr_storage &addr,
                               const bool bProxy /* = false*/) {
#else
void ServiceLoop::AcceptSocket(const int s, const sockaddr_storage &addr,
  ...
  if (pNewSocket == NULL) {
    ...
    AppendDebugLog(
        "%s - [MEM] Cannot allocate pNewSocket in ServiceLoop::AcceptSocket\n");
    return;
  }
