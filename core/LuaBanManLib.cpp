...
static int GetPermBan(lua_State * pLua) {
	...
	BanItem * pBan = BanManager::m_Ptr->FindPermNick(sValue, szLen);
    ...
    if(HashIP(sValue, ui128Hash) == true) {
		...
		pBan = BanManager::m_Ptr->FindPermIP(ui128Hash);
        if(pBan != NULL) {
#if LUA_VERSION_NUM < 503
			lua_pushnumber(pLua, ++i);
#else
...
static int GetTempBan(lua_State * pLua) {
	...
	BanItem * pBan = BanManager::m_Ptr->FindTempNick(sValue, szLen);
    ...
    if(HashIP(sValue, ui128Hash) == true) {
		...
		pBan = BanManager::m_Ptr->FindTempIP(ui128Hash, acc_time);
        if(pBan != NULL) {
#if LUA_VERSION_NUM < 503
			lua_pushnumber(pLua, ++i);
#else
