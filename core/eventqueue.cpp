...
EventQueue::Event::Event()
    : m_pPrev(NULL), m_pNext(NULL), m_sMsg(NULL), m_ui8Id(0) {
  memset(&m_ui128IpHash, 0, 16);
};
//---------------------------------------------------------------------------

EventQueue::EventQueue()
    : m_pNormalE(NULL), m_pThreadE(NULL), m_pNormalS(NULL), m_pThreadS(NULL) {
#ifdef _WIN32
...
EventQueue::~EventQueue() {
  Event *cur = NULL, *next = m_pNormalS;
  ...
  while (next != NULL) {
    ...
    if (cur->m_sMsg != NULL) {
      if (HeapFree(ServerManager::m_hPtokaXHeap, HEAP_NO_SERIALIZE,
                   (void *)cur->m_sMsg) == 0) {
        AppendDebugLog("%s - [MEM] Cannot deallocate cur->m_sMsg in "
                       "EventQueue::~EventQueue\n");
      }
    }
#else
...
void EventQueue::AddNormal(const uint8_t ui8Id, char *sMsg) {
  ...
  if (pNewEvent == NULL) {
    AppendDebugLog(
        "%s - [MEM] Cannot allocate pNewEvent in EventQueue::AddNormal\n");
    return;
  }
  ...
  if (sMsg != NULL) {
...
#ifdef _WIN32
    pNewEvent->m_sMsg = (char *)HeapAlloc(ServerManager::m_hPtokaXHeap,
    ...
    if (pNewEvent->m_sMsg == NULL) {
      ...
      AppendDebugLogFormat("[MEM] Cannot allocate %zu bytes for "
                           "pNewEvent->m_sMsg in EventQueue::AddNormal\n",
                           szLen + 1);
...
void EventQueue::AddThread(const uint8_t ui8Id, char *sMsg,
                           const sockaddr_storage *sas /* = NULL*/) {
  Event *pNewEvent = new (std::nothrow) Event();

  if (pNewEvent == NULL) {
    AppendDebugLog(
        "%s - [MEM] Cannot allocate pNewEvent in EventQueue::AddThread\n");
    return;
  }
  ...
  if (sMsg != NULL) {
    ...
    if (pNewEvent->m_sMsg == NULL) {
      ...
      AppendDebugLogFormat("[MEM] Cannot allocate %zu bytes for "
                           "pNewEvent->m_sMsg in EventQueue::AddThread\n",
                           szLen + 1);
  ...
  if (sas != NULL) {
    if (sas->ss_family == AF_INET6) {
      memcpy(pNewEvent->m_ui128IpHash,
             &((struct sockaddr_in6 *)sas)->sin6_addr.s6_addr, 16);
    } else {
      memset(pNewEvent->m_ui128IpHash, 0, 16);
      pNewEvent->m_ui128IpHash[10] = 255;
      pNewEvent->m_ui128IpHash[11] = 255;
      memcpy(pNewEvent->m_ui128IpHash + 12,
             &((struct sockaddr_in *)sas)->sin_addr.s_addr, 4);
    }
  } else {
    memset(pNewEvent->m_ui128IpHash, 0, 16);
  }
...
void EventQueue::ProcessEvents() {
  ...
  while (next != NULL) {
    ...
    case EVENT_RSTSCRIPT: {
      ...
      if (pScript == NULL || pScript->m_bEnabled == false ||
          pScript->m_pLua == NULL) {
        return;
      }
  ...
  while (next != NULL) {
    ...
    case EVENT_UDP_SR: {
      ...
      if (::SendMessage(
        ...
        char sMsg[128];
        int iMsgLen = snprintf(sMsg, 128, "UDP > %s (%s) > ", pUser->m_sNick,
                               pUser->m_sIP);
        if (iMsgLen > 0) {
          RichEditAppendText(
              MainWindowPageUsersChat::m_Ptr
                  ->m_hWndPageItems[MainWindowPageUsersChat::REDT_CHAT],
              (string(sMsg, iMsgLen) + cur->m_sMsg).c_str());
        }
      }
#endif
