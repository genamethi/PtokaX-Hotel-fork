...
function M.select_instance(cb)
	...
	if #found == 0 then
		return utils.warn("no ptokax instances found")
	end
	if #found == 1 then
		M.instance = found[1]
		return cb(found[1])
...
function M.pick_instance()
	M.instance = nil
	M.select_instance(function(instance)
		utils.info("instance: %s", instance)
	end)
end
...
function M.journal()
	M.with_instance(function(instance)
		utils.term_split({
			"journalctl",
			"-u",
			string.format(M.unit, instance),
			"PTOKAX_SUBSYSTEM=console",
			"-f",
		})
	end)
end
