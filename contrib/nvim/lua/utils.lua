--lifted from https://github.com/ibhagwan/nvim-lua/
...
function M.notify(lvl, ...)
	...
	local function nvim_echo()
		...
		if echo_opts.err and _G.fzf_jobstart and #vim.api.nvim_list_uis() == 0 then
			local output = vim.tbl_map(function(chunk)
				return chunk[1]
			end, chunks)
...
--- Write to a unix stream socket, elevating only when the socket denies access
--- @param path string
--- @param text string
--- @param opts? table sudo: skip the unprivileged attempt, timeout: connect ms
--- @return boolean
M.socket_write = function(path, text, opts)
	opts = opts or {}
	...
	if not opts.sudo then
		...
		-- anything but a permission problem will not be fixed by sudo either
	...
	if vim.fn.executable("socat") == 0 then
		M.warn("socat is required to write to a root owned socket")
		return false
	end
	...
	-- the redirect belongs to the inner shell, sudo owns the outer stdin
	local inner = string.format("socat - UNIX-CONNECT:%s < %s", path, tmpfile)
	local ok = M.sudo_sh(inner, { cache = opts.cache })
	vim.fn.delete(tmpfile)
	return ok
end

--- Run one shell command under sudo, prompting unless a cached timestamp is
...
M.sudo_sh = function(cmd, opts)
	...
	vim.list_extend(argv, { "sh", "-c", cmd })
	local res = vim.system(argv, { stdin = password .. "\n" }):wait()
	if res.code ~= 0 then
...
--- @param cmd string[]
--- @param opts? table height: lines, focus: enter insert in the terminal
M.term_split = function(cmd, opts)
	opts = opts or {}
	vim.cmd(string.format("botright %dnew", opts.height or 15))
...
--- Instance names of a systemd template unit, e.g. M.systemd_instances("ptokax")
--- @param template string
--- @return string[]
M.systemd_instances = function(template)
	local res = vim.system({
		...
		"--no-pager",
		"--plain",
		string.format("%s@*.service", template),
	}):wait()
	local found = {}
	for line in (res.stdout or ""):gmatch("[^\n]+") do
		local name = line:match(string.format("^%%s*%s@([%%w_%%-%%.]-)%%.service", template))
		if name then
			found[#found + 1] = name
		end
	end
